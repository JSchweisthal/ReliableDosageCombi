...
#########################################################
    ...
    for k in range(policy_args.k_runs):
        ...
        model_outcome = DCNet(outcome_args.model_args)
        model_outcome.load_state_dict(torch.load(os.path.join(save_path, 'model_outcome_final.pth.tar'))['state_dict'])
        model_outcome.eval()
        ...
        for epoch in epochs:
            ...
            for idx, (dataset_index, (inputs, y)) in enumerate(train_loader):
                ...
                out, hidden = model_outcome.forward(t_param + 0.5, x)
    ...
    if policy_args.unadjusted:
        df_runs.to_pickle(os.path.join(save_path, 'runs_unadjusted.pkl'))
        torch.save(runs_best_model.state_dict(), os.path.join(save_path, f'model_policy_unadjusted.pth.tar'))
    else:
        df_runs.to_pickle(os.path.join(save_path, 'runs.pkl'))
        torch.save(runs_best_model.state_dict(), os.path.join(save_path, f'model_policy.pth.tar'))
