...
from src.models.dynamic_net import DCNet 
...
#########################################################
    ...
    model_args.dim_cov = train_matrix.shape[1]-model_args.dim_treat-1
    ...
    for epoch in epochs:
        ...
        for idx, (inputs, y) in enumerate(train_loader):
            t =  inputs[:, :model_args.dim_treat]
            x = inputs[:, model_args.dim_treat:]

            optimizer.zero_grad()
            out, hidden = model.forward(t, x)
        ...
        if ((epoch+1) % verbose == 0): 
            _, mse_out = eval_outcome(model, test_matrix[:, model_args.dim_treat:-1], data_eval_test)
            mse_out = float(mse_out)

            writer.add_scalar(f"{dataset}/{model_args.dim_treat}dim/4_MISE_out", mse_out, epoch)
            print("Train loss: {:.3f} --- Validation loss: {:.3f}".format(running_loss, val_loss)) 
            print("MISE out: {:.3f}".format( mse_out) )
            print('-----------------------------------------------------------------')
            ...
            }, 'model_outcome_'+str(epoch+1), checkpoint_dir=save_path)

            print('-----------------------------------------------------------------')
    _, mse_out = eval_outcome(best_model, test_matrix[:, model_args.dim_treat:-1], data_eval_test)
    mse_out = float(mse_out)
    ...
    save_checkpoint({
            ...
            }, 'model_outcome_final', checkpoint_dir=save_path)
    print(f"Save final model to: {os.path.join(save_path, f'model_outcome_final')}")
